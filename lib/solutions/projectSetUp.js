...
var projectQuestions    = require('../questions/project');
var projectSolution     = require('./project');
...
module.exports = function( answers ){
    ...
    clientLibs( function( libs ){
        ...
        npm.load( function(){
            ...
            npm.commands.install("", ['gulp', 'browserify', 'babel', 'babelify'], function( err ){

                if( err ){
                    console.error(err);
                }

                var rootPackageFile = templates.project['package-root'];
                var rootPackage     = templateRenderer( 'package.json', rootPackageFile.value, { project_name : project.name } );
                try {
                ...
                inquirer.prompt( [
                    {
                        type: 'confirm',
                        name: 'continue',
                        message : 'Would you like to create a new component or client library?'
                    }
                ], function( lastAnswers ){

                    if( lastAnswers.continue === true ){
                        inquirer.prompt( projectQuestions, projectSolution );
                    }
